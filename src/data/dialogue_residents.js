...
//
// 원칙
//   · 참고자료의 성격·직업·대사를 따른다. 화학이 성격으로 번역되는 게 이 게임이다
//   · 마지막 선택지에서 힘(element)을 빌려준다. 다만 귀족 기체(Ne·Ar)는 반응하지
//     않는 것이 정체성이라 빌려주지 않는다 — 규칙이 곧 화학이다
//   · 대사 안에 실제 화학을 한 줄씩 심는다 (얀-텔러, 쿨롱 장벽, 희생 양극 …)
//
// 구조는 dialogue.js와 같다.
...
export const RESIDENT_DIALOGUES = {
  ...
  ar: {
    ...
    nodes: {
      ...
      intro: {
        lines: [
          "…zzz. …응? 아, 손님이야? 아르곤이야~ 아르곤 시티 촌장. 지금은 출장 중… 이랄까, 낮잠 중.",
  ...
  ne: {
    ...
    nodes: {
      ...
      intro: {
        lines: [
          "최고재판소 소장 네온이다. 여기서 뭘 하냐고? 순회 재판. 변두리도 법정은 필요하지.",
  ...
  sb: {
    ...
    nodes: {
      intro: {
        lines: [
          "밤길에 혼자라니, 위험하지 않니? …아, 나는 안티모니. 스티비 대성당 수도사야.",
  ...
  ni: {
    ...
    nodes: {
      intro: {
        lines: [
          "강철부대 대장 니켈이다. …코발트는 대식가가 분명해. 방금 또 뭘 먹더군.",
  ...
  be: {
    ...
    nodes: {
      intro: {
        lines: [
          "쿨롱 장벽… 까다롭다니까. 마지막 한 걸음을 어떻게 다가가면 좋을까?",
          "…어머, 손님? 베릴륨이에요. 물리학자. 쿠키도 굽고요. 하나 드실래요?",
        ],
        choices: [
          { text: "쿨롱 장벽이 뭐예요?", to: "barrier", effect: { codex: "be" } },
          { text: "쿠키 주세요", to: "barrier", effect: { codex: "be" } },
        ],
      },
      barrier: {
        lines: [
          "양성자끼리는 서로 밀어내요. 그래서 원자핵 둘을 붙이려면 그 미는 힘을 넘어야 하죠. 그게 쿨롱 장벽.",
  ...
  cu: {
    ...
    nodes: {
      intro: {
        lines: [
          "어라, 혹시 키가 컸나? 얀-텔러 효과인가! …아, 사람이구나. 구리야. 이 근처에 살아.",
          "옛날엔 귀금속 형제였는데, 아주 오래전에 헤어졌어. 지금은 그냥 마을 사람. 전깃줄도 해 주고 냄비도 해 주고.",
        ],
        choices: [
          { text: "금과 은이 형제라고요?", to: "brothers", effect: { codex: "cu" } },
          { text: "얀-텔러가 뭐예요?", to: "brothers", effect: { codex: "cu" } },
        ],
      },
      brothers: {
        lines: [
          "11족 삼형제. 금·은·구리. 전자 배치가 같아서 성질이 닮았어. 잘 늘어나고 전기를 잘 통하고.",
          "얀-텔러는… 내 이온이 찌그러지는 거야. 대칭이면 불안정해서. 형제들 없으면 나도 좀 찌그러져 있지, 하하.",
  ...
  ag: {
    ...
    nodes: {
      intro: {
        lines: [
          "…이 숲까지 오는 사람은 드문데. 은이야. 저 너머에 내가 만든 거울 나라가 있어.",
  ...
  rn: {
    ...
    nodes: {
      intro: {
        lines: [
          "헤헤, 온천 리조트 '호르메시스'에 오신 걸 환영합니다~ 라돈이에요. 이 사탕 줄까?",
      ...
      spa: {
        lines: [
          ...
          "조금은 몸에 좋다고 하고, 많이는 안 좋고. 그게 호르메시스야. 뭐든 양이 문제라니까.",
